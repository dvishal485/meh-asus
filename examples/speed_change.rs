...
use anyhow::Result;
use meh_asus::{fan::AsusNbWmiFanMode, pwm_enable::PwmEnableAbstraction};



macro_rules! print_config {
    ($($x:expr),+ $(,)?) => {
        $( println!("{} => {}", $x.get_label().unwrap(), $x.get_fan_mode().unwrap()); )+
    };
}

fn main() -> Result<()> {
    let mut fan1 = PwmEnableAbstraction::find_and_get_read_only(1)?;
    let mut fan2 = PwmEnableAbstraction::find_and_get_read_only(1)?;

    // (_, _)
    println!("{}", fan1.get_fan_mode()?);
